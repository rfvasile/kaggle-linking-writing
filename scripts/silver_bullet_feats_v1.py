...
def dev_feats(df):
    ...
    temp = temp.group_by('id').agg(pl.col('text_change').str.join('').str.extract_all(r'q+'))
    ...
    temp = df.group_by("id").agg(pl.sum('action_time').name.suffix('_sum'), pl.mean(*num_cols).name.suffix('_mean'), pl.col(*num_cols).std().name.suffix('_std'),
                                 pl.median(*num_cols).name.suffix('_median'), pl.min(*num_cols).name.suffix('_min'), pl.max(*num_cols).name.suffix('_max'),
                                 pl.col(*num_cols).quantile(0.5).name.suffix('_quantile'))
    feats = feats.join(temp, on='id', how='left')
